...
console.log("actif          EXH  | croisement (W/L) | deux-SEP (W/L)   SELL-SEP");
let TcW=0,TcL=0,TsW=0,TsL=0,nAssetsWithExh=0;
for(const a of assets){
  ...
  let lastEp=-1e9;let cW=0,cL=0,sW=0,sL=0,ssW=0,ssL=0,tot=0;
  for(let i=0;i<rows.length;i++){
    ...
    const obs=observeProfile(det);const out=outcome(series,i,side,series[i].atr);if(!out)continue;tot++;
    const hasCross=obs.contact==="CONTACT"||obs.contactPrev==="CONTACT";
    if(hasCross){out==="WIN"?cW++:cL++;}else{out==="WIN"?sW++:sL++;if(side==="SELL")(out==="WIN"?ssW++:ssL++);}
  }
  if(tot===0)continue;nAssetsWithExh++;TcW+=cW;TcL+=cL;TsW+=sW;TsL+=sL;
  console.log(`${a.padEnd(14)} ${String(tot).padStart(3)}  |   ${String(cW+cL).padStart(2)} (${cW}/${cL})       |  ${String(sW+sL).padStart(2)} (${sW}/${sL})       ${ssW+ssL?`${ssW}/${ssL}`:""}`);
}
console.log(`\n${nAssetsWithExh}/19 actifs ont des EXH.  Croisement présent: ${TcW}/${TcL} (${TcW+TcL})  ·  deux-SEP: ${TsW}/${TsL} (${TsW+TsL})`);
