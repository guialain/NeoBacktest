...
for(const a of assets){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    const k=`contactPrev=${obs.contactPrev}`;
    add(byCP.all,k,out);add(byCP[side],k,out);
  }
}
const wr=b=>{const n=b.WIN+b.LOSS;return n?`${(100*b.WIN/n).toFixed(0).padStart(3)}% (${b.WIN}W/${b.LOSS}L, n=${n})`:"—";};
console.log("=== EXH (tous côtés) — par contactPrev (croisement frais ?) ===");
for(const [k,b] of Object.entries(byCP.all).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
console.log("  -- SELL --");for(const [k,b] of Object.entries(byCP.SELL).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
console.log("  -- BUY --");for(const [k,b] of Object.entries(byCP.BUY).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
