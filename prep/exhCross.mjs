...
for(const a of assets){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    const k = hasCross ? "croisement présent" : "deux SEPARATED (vide)";
    add(B.all,k,out);add(B[side],k,out);
    if(hasCross)kept++;else removed++;
  }
}
const wr=b=>{const n=b.WIN+b.LOSS;return n?`${(100*b.WIN/n).toFixed(0).padStart(3)}% (${b.WIN}W/${b.LOSS}L, n=${n})`:"—";};
console.log("=== EXH — croisement présent vs deux SEPARATED ===");
for(const [k,b] of Object.entries(B.all).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
console.log("  -- SELL --");for(const [k,b] of Object.entries(B.SELL).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
console.log("  -- BUY --");for(const [k,b] of Object.entries(B.BUY).sort())console.log(`  ${k.padEnd(24)} ${wr(b)}`);
console.log(`\nSi on exige un croisement (contact=CONTACT OU contactPrev=CONTACT) : ${kept} EXH gardés, ${removed} filtrés.`);
