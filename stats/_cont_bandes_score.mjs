// _cont_bandes_score.mjs — WR DU RANG ③ PAR TRANCHE DE SCORE, AU PAS DE 1, ET PAR COTE.
// ============================================================================================
// ⚠⚠ DERIVE DE `_exh_bandes_score.mjs` (20/08) — MEME FORME, MAIS **PAS LA MEME MECANIQUE** :
//   au rang ③ le VETO est teste **APRES** le seuil (`cont-below-min` puis `cont-vetoed`). Baisser
//   `MIN_CONT` ouvre donc les bandes basses, mais **les 18 vetos du ③ continuent d'y mordre** :
//   une bande vide sous 18 ne veut pas dire « personne », elle veut dire « personne N'A SURVECU
//   AUX VETOS ». C'est l'inverse du rang ①, ou le seuil est le dernier mot.
// ⚠⚠ ET LES TIRS SONT CONCURRENTS : `MIN_CONT=0` n'est PAS le carnet de prod avec des bandes en
//   plus — c'est un AUTRE run. Mesure du jour : passer de 22 a 18 deplacait deja 343 tirs.
// ⚠ `sc.cont` est le score BONIFIE, la quantite que le seuil compare. Il est SIGNE (le cote vient
//   du signe) ⇒ on range sur `Math.abs`.
...
//   TIRE PAS quand `MIN_CONT = 16`, donc elle n'a ni R ni outcome. `MIN_CONT=15` est donc OBLIGATOIRE
...
// ⚙ Usage : `MIN_CONT=15 MAXOPEN=100 MAXPERSYMBOL=100 node stats/_exh_bandes_score.mjs`
import fs from "fs";
...
const EXH = sig.filter((t) => t.strategy === "CONT" && typeof t.R === "number");
const lisible = EXH.filter((t) => Number.isFinite(t.sc?.cont));
...
console.log(`   MIN_CONT ${MIN_CONT}  ·  capacite maxOpen ${MAXOPEN} / maxPerSymbol ${MAXPERSYMBOL}  ·  NO_TRIGGER=1`);
console.log(`   tirs CONT ${EXH.length}  ·  score lisible ${lisible.length}` +
