...
for(const ag of [0,1,2]){
  const g=all.filter(s=>s.crossAge===ag);
  console.log(`### crossAge ${ag} — n=${g.length}`);
  st(g.filter(s=>s._prodSH!=null&&s._prodSH<0), "TURN (prod<0)");
  st(g.filter(s=>s._prodSH!=null&&s._prodSH>0), "pas turn (prod>0)");
}
