...
const CH = ["timestamp", "ts_utc", "price", "symbol",
            "zscore_h1_s0", "sigma_h1", "zscore_h4_s0", "sigma_h4"];
const S = {};
for (const tf of ["H1", "H4"]) S[tf] = { abs: [], bandes: {}, tr: 0, n: 0, hors: 0 };
for (const tf of ["H1", "H4"]) for (const b of GAP_LEVELS) S[tf].bandes[b] = 0;
...
for (const f of fs.readdirSync(DIR).filter((x) => x.endsWith(".csv"))) {
  ...
  const cur = { H1: null, H4: null };
  let prev = null;
  for (const r of rows) {
    if (r.panne) { cur.H1 = cur.H4 = null; prev = null; continue; }
    const rupt = prev != null && r.u - prev > TROU;
    if (rupt) { cur.H1 = cur.H4 = null; }
    else if (prev != null) minutes += r.u - prev;
    prev = r.u;
    nL++;
    const atr = atrP50Price(r.sym, r.p);
    if (!(atr > 0)) continue;
    for (const [tf, z, s] of [["H1", r.z1, r.s1], ["H4", r.z4, r.s4]]) {
...
console.log("  " + "bande".padEnd(14) + "H1".padStart(10) + "H4".padStart(10));
for (const b of GAP_LEVELS)
  console.log("  " + b.padEnd(14) + pc(S.H1.bandes[b], S.H1.n).padStart(10) + pc(S.H4.bandes[b], S.H4.n).padStart(10));
console.log("  " + "hors bande".padEnd(14) + pc(S.H1.hors, S.H1.n).padStart(10) + pc(S.H4.hors, S.H4.n).padStart(10));
const haut = (tf) => ["TENSE_HIGH", "EXTREME", "SNAPPED"].reduce((a, b) => a + S[tf].bandes[b], 0);
console.log(`  ⭐ moitie HAUTE (TENSE_HIGH+EXTREME+SNAPPED) : H1 ${pc(haut("H1"), S.H1.n)}  ·  H4 ${pc(haut("H4"), S.H4.n)}`);

console.log(`\n  ── ③ PERSISTANCE de la bande (transitions / 1 000 min reelles) ──`);
for (const tf of ["H1", "H4"]) console.log(`  ${tf} : ${(1000 * S[tf].tr / minutes).toFixed(2)}`);
